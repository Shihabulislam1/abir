...
class LaneDetector:
    ...
    def detect(self, frame: np.ndarray) -> LaneDetection:
        ...
        if getattr(self.cfg, "clahe_enabled", True):
            clahe = cv2.createCLAHE(
                clipLimit=getattr(self.cfg, "clahe_clip", 2.5),
                tileGridSize=(getattr(self.cfg, "clahe_tile", 8), getattr(self.cfg, "clahe_tile", 8)),
            )
    ...
    def _pick_peaks(self, hist: np.ndarray, frame_w: int) -> List[int]:
        ...
        edge = max(1, int(frame_w * getattr(self.cfg, "peak_edge_mask_frac", 0.05)))
        smooth[:edge] = 0
        smooth[-edge:] = 0
        min_count = max(getattr(self.cfg, "window_minpix", 40), 10)
        min_sep = getattr(self.cfg, "min_peak_separation", 90)
    ...
    def _track(
        ...
        for w_idx in range(getattr(self.cfg, "nwindows", 12)):
